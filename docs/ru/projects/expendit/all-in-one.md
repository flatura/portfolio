# Внутренняя система учёта расходных материалов — всё вместе

Эта страница собирается из компактных разделов проектной документации. Отдельные файлы разделов остаются источником истины; эта страница предназначена для последовательного чтения, ревью и экспорта в стиле PDF.

## Содержание

- [Краткое описание](#_2)
- [Обзор](#_8)
- [Контекст и проблема](#_11)
- [Цели, требования и ограничения](#_14)
- [Роль и обязанности](#_19)
- [Модель системы](#_20)
- [Архитектура и интеграции](#_24)
- [Безопасность, качество и эксплуатация](#_28)
- [Решения, компромиссы и риски](#_33)
- [Дорожная карта и демонстрация](#_37)
- [Architecture Decision Records](#architecture-decision-records)

{% include-markdown "./summary.md" heading-offset=1 %}
{% include-markdown "./01-overview.md" heading-offset=1 %}
{% include-markdown "./02-context-and-problem.md" heading-offset=1 %}
{% include-markdown "./03-goals-requirements-and-constraints.md" heading-offset=1 %}
{% include-markdown "./04-role-and-responsibilities.md" heading-offset=1 %}
{% include-markdown "./05-system-model.md" heading-offset=1 %}
{% include-markdown "./06-architecture-and-integrations.md" heading-offset=1 %}
{% include-markdown "./07-security-quality-and-operations.md" heading-offset=1 %}
{% include-markdown "./08-decisions-trade-offs-and-risks.md" heading-offset=1 %}
{% include-markdown "./09-roadmap-and-demonstration.md" heading-offset=1 %}

## Architecture Decision Records

См. [Architecture Decision Records](adr/index.md).
